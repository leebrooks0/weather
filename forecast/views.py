...
def forecast_view(request, city):
    ...
    if not days:
        return JsonResponse({"error": "`days` is a required query string parameter"}, status=HTTPStatus.BAD_REQUEST)
    try:
        forecast = weather.get_daily_forecasts(city, days)
    except CityNotFound:
        return JsonResponse({"error": "`city` not found"}, status=HTTPStatus.BAD_REQUEST)
